...
var ship_controller_is_assignment_done = false
...
var ship_bullet_decreased_rotation_speed_time_left = 0.0
var ship_bullet_decreased_rotation_speed_time = 0.1
...
func handle_controller_joy_axis_input(delta: float):
	...
	if (jv.length() > 0.2):
		...
		if ship_bullet_decreased_rotation_speed_time_left > 0:
...
func playground_add_bullet(_delta: float):
	...
	ship_bullet_decreased_rotation_speed_time_left = ship_bullet_decreased_rotation_speed_time
...
func playground_add_missile(_delta: float):
	...
	if (is_instance_valid(missile_possible_target) and 
		missile_possible_target.is_in_group("Spaceship")):
...
func _physics_process(delta: float) -> void:
	...
	if ship_is_returning_to_playground:
		...
		if ship_controller_is_assignment_done:
			if Input.is_joy_button_pressed(ship_controller_device, 
	...
	if ship_bullet_decreased_rotation_speed_time_left > 0:
		ship_bullet_decreased_rotation_speed_time_left -= delta
	if ship_space_mine_cooldown_time_left > 0:
